...
#First, check to see if tor is installed. If not then clean the environment and install packages
#Then, if tor is installed skip to torsocks curl and reconfigure firefox-esr 

ISTORINSTALLED=/usr/bin/tor
if [ ! -f "$ISTORINSTALLED" ]; then
...
if [ -f "$ISTORINSTALLED" ]; then
    ...
    #TODO: Torsocks may not be immediately usable. Investigate whether we should wait some seconds.
